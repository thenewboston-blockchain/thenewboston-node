...
def make_url_path(block_chunk_meta):
    file_path = block_chunk_meta.blockchain_root_relative_file_path
    return os.path.join(settings.BLOCKCHAIN_URL_PATH_PREFIX, file_path) if file_path else None


class BlockChunksMetaSerializer(CustomSerializer):
    start_block_number = fields.IntegerField()
    end_block_number = fields.IntegerField()
    url_path = fields.SerializerMethodField()
    urls = fields.SerializerMethodField()

    def get_url_path(self, block_chunk_meta):
        return make_url_path(block_chunk_meta)

    def get_urls(self, block_chunk_meta):
        url_path = make_url_path(block_chunk_meta)
        if url_path is None:
            return None

        blockchain = block_chunk_meta.blockchain

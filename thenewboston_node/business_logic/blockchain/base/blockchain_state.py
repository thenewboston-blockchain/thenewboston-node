...
class BlockchainStateMixin(BaseMixin):
    ...
    def generate_blockchain_state(self, last_block_number: Optional[int] = None) -> BlockchainState:
        ...
        primary_validator = self.get_primary_validator(block_number=last_block_number)  # type: ignore
        if primary_validator is None:
            raise InvalidBlockchain('Primary validator is required to generate blockchain state')
        blockchain_state = BlockchainState(message=BlockchainStateMessage(account_states=account_states), signer=primary_validator.identifier)

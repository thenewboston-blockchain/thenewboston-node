...
initial_arf = factories.InitialAccountRootFileFactory(
    accounts={USER_ACCOUNT_1: factories.AccountBalanceFactory(balance=100, balance_lock=USER_ACCOUNT_1)},
)
...
block_0 = factories.BlockFactory(
    message=factories.BlockMessageFactory(
        ...
        updated_balances={
            USER_ACCOUNT_1: factories.BlockAccountBalanceFactory(balance=89, balance_lock=MESSAGE_HASH),
            USER_ACCOUNT_2: factories.BlockAccountBalanceFactory(balance=11, balance_lock=None),
        }
    ),
...
def test_validate_number_of_accounts_mismatch(blockchain_base):
    arf_0 = factories.AccountRootFileFactory(
        accounts={USER_ACCOUNT_1: factories.AccountBalanceFactory(balance=89, balance_lock=MESSAGE_HASH)}
    )
...
def test_validate_non_existent_account(blockchain_base):
    ...
    arf_0 = factories.AccountRootFileFactory(
        accounts={
            USER_ACCOUNT_1: factories.AccountBalanceFactory(balance=89, balance_lock=MESSAGE_HASH),
            non_existent_account: factories.AccountBalanceFactory(balance=11, balance_lock=non_existent_account),
        }
    )
    arf_patch = patch.object(blockchain_base, 'iter_account_root_files', get_generator([initial_arf, arf_0]))
    block_patch = patch.object(blockchain_base, 'iter_blocks', get_generator([block_0]))
...
def test_validate_balance_value(blockchain_base):
    ...
    arf_0 = factories.AccountRootFileFactory(
        accounts={
            USER_ACCOUNT_1: factories.AccountBalanceFactory(balance=wrong_balance, balance_lock=MESSAGE_HASH),
            USER_ACCOUNT_2: factories.AccountBalanceFactory(balance=11, balance_lock=USER_ACCOUNT_2),
        }
    )
    arf_patch = patch.object(blockchain_base, 'iter_account_root_files', get_generator([initial_arf, arf_0]))
    block_patch = patch.object(blockchain_base, 'iter_blocks', get_generator([block_0]))
...
def test_validate_balance_lock(blockchain_base):
    ...
    arf_0 = factories.AccountRootFileFactory(
        accounts={
            USER_ACCOUNT_1: factories.AccountBalanceFactory(balance=89, balance_lock=wrong_lock),
            USER_ACCOUNT_2: factories.AccountBalanceFactory(balance=11, balance_lock=USER_ACCOUNT_2),
        }
    )
    arf_patch = patch.object(blockchain_base, 'iter_account_root_files', get_generator([initial_arf, arf_0]))

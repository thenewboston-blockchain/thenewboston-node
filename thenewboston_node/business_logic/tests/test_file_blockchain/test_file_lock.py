...
def test_add_blockchain_state_is_thread_safe(forced_file_blockchain, blockchain_directory, blockchain_genesis_state):
    lock_file_path = os.path.join(blockchain_directory, 'file.lock')
    pool = ThreadPoolExecutor()

    with filelock.FileLock(lock_file_path):
        future = pool.submit(forced_file_blockchain.add_blockchain_state, blockchain_genesis_state)
